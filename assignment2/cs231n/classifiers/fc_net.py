    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers - 1):
            if self.normalization == "batchnorm":
                ...
                layer_input, ar_cache[layer] = affine_ln_relu_forward(layer_input,
                                                                      self.params['W%d' % (layer + 1)],
                                                                      self.params['b%d' % (layer + 1)],
                                                                      self.params['gamma%d' % (layer + 1)],
                                                                      self.params['beta%d' % (layer + 1)],
        ...
        for layer in range(self.num_layers - 1, 0, -1):
            loss = loss + 0.5 * self.reg * np.sum(self.params['W%d' % layer] * self.params['W%d' % layer])

            if self.use_dropout:
                dx = dropout_backward(dx, dp_cache[layer - 1])

            if self.normalization == "batchnorm":
                dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dx, ar_cache[layer - 1])
                grads['gamma%d' % layer] = dgamma
                grads['beta%d' % layer] = dbeta
            elif self.normalization == "layernorm":
                dx, dw, db, dgamma, dbeta = affine_ln_relu_backward(dx, ar_cache[layer - 1])
                grads['gamma%d' % layer] = dgamma
                grads['beta%d' % layer] = dbeta
            else:
                dx, dw, db = affine_relu_backward(dx, ar_cache[layer - 1])
