...
def affine_forward(x, w, b):
    ...
    num_data = x.shape[0]
    out = x.reshape(num_data, -1).dot(w) + b
...
def affine_backward(dout, cache):
    ...
    num_data = x.shape[0]
    dw = np.dot(x.reshape(num_data, -1).T, dout)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        if layernorm == 0:
            cache = (gamma, x, sample_mean, sample_var, eps, x_hat, 0)
            running_mean = momentum * running_mean + (1 - momentum) * sample_mean
            running_var = momentum * running_var + (1 - momentum) * sample_var

        if layernorm == 1:
            cache = (gamma, x, sample_mean, sample_var, eps, x_hat, 1)
...
def batchnorm_backward_alt(dout, cache):
    ...
    gamma, x, u_b, sigma_squared_b, eps, x_hat, sum_axis = cache
    N = x.shape[0]
    ...
    dgamma = np.sum(dout * x_hat, axis=sum_axis)
    dbeta = np.sum(dout, axis=sum_axis)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            dx[:, :, i * stride: i * stride + HH, j * stride: j * stride + WW] += temp_binary_mask * \
                                                                                  (dout[:, :, i, j])[:, :, None, None]
